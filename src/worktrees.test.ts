...
// The scan is async (it must never block the extension host), so the whole suite
// runs inside one sequential async IIFE — the blocks share tmp state and must not
// interleave.
void (async () => {
  {
    const root = makeRepo();
    ...
    assert.equal(await isWorktreeRoot(path.join(root, "ws")), true, "ws is a worktree root");
    assert.equal(await isWorktreeRoot(root), false, "main repo (.git dir) is not a worktree");
    assert.equal(await isWorktreeRoot(path.join(root, "sub")), false, "submodule is not a worktree");

    // nestedWorktreesUnder: finds ws, excludes submodule.
    const found = await nestedWorktreesUnder(root);
  ...
  // monorepo/service-geo, NOT of monorepo. The old <root>/.git/worktrees read missed it.
  {
    ...
    const found = (await nestedWorktreesUnder(root)).map((p) => path.resolve(p)).sort();
    const expected = [
  ...
  // (but is structurally `.../.git/modules/<name>`) must NOT be seen as a worktree.
  {
    ...
    assert.equal(await isWorktreeRoot(sub), false, "submodule under a worktrees-named dir is not a worktree");
    fs.rmSync(base, { recursive: true, force: true });
    console.log("ok - submodule under worktrees-named ancestor is not misclassified");
  }

  // The scan runs on every window focus in a monorepo with thousands of
  // directories. It must yield to the event loop while walking, or the extension
  // host is frozen for the whole traversal (measured: 2,493 readdir + 2,703 lstat
  // on a real go.work monorepo). Assert the loop keeps turning DURING the scan.
  {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "cg-wty-"));
    // A tree wide enough that the walk spans several await points.
    for (let i = 0; i < 60; i++) {
      fs.mkdirSync(path.join(root, `pkg-${i}`, "internal", "biz"), { recursive: true });
    }
    let ticks = 0;
    const timer = setInterval(() => ticks++, 1);
    await nestedWorktreesUnder(root);
    clearInterval(timer);
    // A synchronous walk pins the loop and lets zero timers fire.
    assert.ok(ticks > 0, `scan yielded to the event loop (ticks=${ticks})`);
    fs.rmSync(root, { recursive: true, force: true });
    console.log("ok - worktree scan yields to the event loop while walking");
  }

  // Deterministic ordering: the registry's attach cap slices this list, so an
  // unstable order could drop a different worktree on each refresh.
  {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "cg-wtz-"));
    for (const name of ["zulu", "alpha", "mike"]) {
      const admin = path.join(root, ".git", "worktrees", name);
      fs.mkdirSync(admin, { recursive: true });
      fs.mkdirSync(path.join(root, name), { recursive: true });
      fs.writeFileSync(path.join(root, name, ".git"), `gitdir: ${admin}\n`);
    }
    const a = await nestedWorktreesUnder(root);
    const b = await nestedWorktreesUnder(root);
    assert.deepEqual(a, b, "repeated scans return the same order");
    assert.deepEqual(a, [...a].sort(), "scan output is sorted");
    fs.rmSync(root, { recursive: true, force: true });
    console.log("ok - worktree scan order is stable and sorted");
  }
})();
